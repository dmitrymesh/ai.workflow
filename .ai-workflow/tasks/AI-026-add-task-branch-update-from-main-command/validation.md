...
- Unit tests: 28/28 passed (`python -m unittest test_update_from_main -v`)
  ...
  - `update_from_main` handler: requires task_id or --all, mutual exclusion, single task runs process_branch, --all scans all branches, --all skips inactive (done/rejected) branches, main_first mode rejected, exits 1 on conflict
- Dry-run smoke test (`--all`): 5 would-update, 1 skipped-dirty, 2 skipped-no-worktree, 12 skipped-merged — all correct
