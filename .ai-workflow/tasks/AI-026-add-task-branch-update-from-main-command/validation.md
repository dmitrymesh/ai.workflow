...
- Unit tests: 29/29 passed (`python -m unittest test_update_from_main -v`)
  ...
  - `update_from_main` handler: requires task_id or --all, mutual exclusion, single task runs process_branch, --all scans all branches, --all skips inactive (done/rejected) branches with correct two-arg helper call, main_first mode rejected, empty config rejected (defaults to main_first), exits 1 on conflict
